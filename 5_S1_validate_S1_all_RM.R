...
Raam.brp<-shapefile("/media/coleen/DDrive/1_OWASIS/4_Papers/traffic/data/shp_wgs/RAAM_BRP_all_stations_v2_dis_WGS.shp")
...
Raam.net<-shapefile("/media/coleen/DDrive/1_OWASIS/4_Papers/traffic/data/shp_wgs/SM_Stations_BOFEK_WGS")
...
setwd("/media/coleen/DDrive/1_OWASIS/4_Papers/traffic/data/insitu_soil_moisture/rm_ts_all_daily")
...
Raam.time.seq<-as.data.frame(seq(as.Date('2016-04-05'), as.Date('2017-12-31'), by = "days"))
...
setwd("//media/coleen/DDrive/1_OWASIS/4_Papers/traffic/data/shp_wgs/buf/")
...
RM.S1.xts.sub<-lapply(RM.S1.xts, function(x) window(x, start = as.Date("2016-04-05"), end =as.Date("2017-12-31")))
...
#subset time
RM.S1.fld.xts.sub<-lapply(RM.S1.fld.xts, function(x) window(x, start = as.Date("2016-04-05"), end =as.Date("2017-12-31")))
...
RM.S1.fld.mean.xts.sub<-window(RM.S1.fld.mean.xts, start = as.Date("2016-04-05"), end =as.Date("2017-12-31"))
...
#subset time
RM.S1.pt.center.xts.sub<-lapply(RM.S1.pt.center.xts, function(x) window(x, start = as.Date("2016-04-05"), end =as.Date("2017-12-31")))
...
names(RM.S1.fld.mean.xts.sub)<-"ras10_fld_mean"
names(RM.S1.fld.xts.sub)<-c("ras50_fld","ras100_fld","ras120_fld","ras150_fld")


RM.S1.all<-c(RM.S1.xts.sub,RM.S1.fld.xts.sub,RM.S1.pt.center.xts.sub,RM.S1.fld.mean.xts.sub)
names(RM.S1.all)
...
for(i in 1:ncol(RM.situ.sub.xts)){
  ...
  plot(as.vector(RM.situ.sub.xts[,i]),as.vector(RM.S1.sub[,i]), xlim = c(0, 0.5), ylim = c(0,0.5), pch = 19, ylab= "Soil Moisture S1",xlab= "Soil Moisture Raam Network",
       main = paste(names(RM.S1.all[[h]][,i]), names(RM.S1.all[h])),col = alpha(pt.colors[[h]],0.5))
  abline(0,1, lty = 2)
...
colnames(rmse.all)<-names(RM.S1.all)
rmse.all$Station<-as.factor(names(RM.S1.all[[1]]))

cor.all<-data.frame(do.call("cbind",cor.list))
colnames(cor.all)<-names(RM.S1.all)
cor.all$Station<-as.factor(names(RM.S1.all[[1]]))

bias.all<-data.frame(do.call("cbind",bias.list))
colnames(bias.all)<-names(RM.S1.all)
bias.all$Station<-as.factor(names(RM.S1.all[[1]]))


unb_rmse.all<-data.frame(do.call("cbind",unb_rmse.list))
colnames(unb_rmse.all)<-names(RM.S1.all)
unb_rmse.all$Station<-as.factor(names(RM.S1.all[[1]]))


colors<-c("blue","green", "red")
a<-length(RM.S1.xts.sub)
b<-length(RM.S1.fld.xts.sub)
d<-length(RM.S1.fld.mean.xts.sub)
c<-length(RM.S1.pt.center.xts.sub)
...
write.csv(cor.all, file = "/media/coleen/DDrive/1_OWASIS/4_Papers/traffic/data/S1_valid_tables/cor.all.rm3.csv",row.names=FALSE)
write.csv(rmse.all, file = "/media/coleen/DDrive/1_OWASIS/4_Papers/traffic/data/S1_valid_tables/rmse.all.rm3.csv",row.names=FALSE)
write.csv(unb_rmse.all, file = "/media/coleen/DDrive/1_OWASIS/4_Papers/traffic/data/S1_valid_tables/unb_rmse.all.rm3.csv",row.names=FALSE)
write.csv(bias.all, file = "/media/coleen/DDrive/1_OWASIS/4_Papers/traffic/data/S1_valid_tables/bias.all.rm3.csv",row.names=FALSE)
